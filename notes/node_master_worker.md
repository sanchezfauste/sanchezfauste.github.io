...
const runMasterWorker = () => {
  ...
  if (cluster.isMaster) {
    ...
    cluster.on("exit", (worker, code, signal) => {
      if (code === 0) {
        console.info(
          `[MASTER] Worker ${worker.id} exited with code '${code}'.`
        );
      } else {
        console.error(
          `[MASTER] Worker ${
    ...
    process.on("message", msg => {
      switch (msg.command) {
        case COMMANDS.SHUTDOWN:
          console.info(
            `[WORKER ${cluster.worker.id}] Master has tell me to shutdown.`
          );
          process.exit(0);
        case COMMANDS.WORK:
          console.info(
            `[WORKER ${cluster.worker.id}] Received work from master.`
          );
          doWork(msg.data)
            .then(() =>
              process.send({
            ...
            .catch(err => {
              console.error(
                `[WORKER ${cluster.worker.id}] Error doing work:`,
                err
              );
              process.exit(1);
            });
